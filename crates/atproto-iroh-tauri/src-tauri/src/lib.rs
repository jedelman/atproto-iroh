...
//!
//! **Split into `lib.rs` + `main.rs` for mobile (2026-09-23).** Tauri's
//! Android/iOS build links this crate as a `cdylib`/`staticlib` and calls
//! `run()` from Java/Swift — there's no `main()` on mobile at all, which
//! is why `cargo tauri android init` needs the app logic reachable from a
//! library crate rather than only a `bin`'s `main.rs`. Desktop still goes
//! through `main.rs`'s ordinary `fn main()`, which just calls `run()`
//! here — same code path either way, only the entry point differs.
